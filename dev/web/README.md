...
A disposable [Penpot](https://penpot.app) stack for drawing Panel UI
mockups. The stack itself is
[penpot-local-stack](https://github.com/oberon-systems/penpot-local-stack),
pinned in `requirements.txt` and installed by `make install`, so this directory
...
`designer@example.com` with password `penpot-local`.
...
Drop a key to fall back to the package default, which is what the compose
project name (`penpot-local`) and the profile password (`penpot-local`) do
here - neither is worth keeping in the repository. Every key also answers to an
...
- Images and icons must be SVG. An export that holds raster images is
  refused and the offending files are listed, so you can replace them and
  export again.
...
After later `up` runs, reconnect `penpot` from `/mcp` instead of
restarting. While the stack is down, the server just shows as failed.
